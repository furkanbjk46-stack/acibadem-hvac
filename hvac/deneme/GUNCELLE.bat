...
echo       ─────── Kod Dosyaları ───────
for %%F in (main_portal.py app_portal.py run_portal.py ai_progress.py daily_report.py monthly_summary_report.py location_manager.py cloud_sync.py portal_watchdog.py) do (
    if exist "%%F" (
        echo       ✅ %%F — mevcut
    ) else (
        echo       ❌ %%F — EKSİK!
