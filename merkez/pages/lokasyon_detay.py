...
with tab1:
    ...
    # ══════════════════════════════════════════════════════
    # BAKIM LİSTESİ DÜZENLE (expander)
    # ══════════════════════════════════════════════════════
    st.markdown("<div style='margin-top:6px'></div>", unsafe_allow_html=True)
    with st.expander("✏️ Bakım & Arıza Listesini Düzenle", expanded=False):

        # Session state'e yükle — lokasyon değişince sıfırla
        _lok_key = f"bakim_edit_lok_{lok_id}"
        if "bakim_edit_list" not in st.session_state or st.session_state.get("bakim_edit_lok") != lok_id:
            st.session_state["bakim_edit_list"] = [
                dict(c) if isinstance(c, dict) else {"ad": str(c), "bakim_turu": "", "detay": ""}
                for c in bakim_list
            ]
            st.session_state["bakim_edit_lok"] = lok_id

        edit_list = st.session_state["bakim_edit_list"]

        # ── Mevcut cihazlar ──────────────────────────────
        if edit_list:
            _h1, _h2, _h3, _h4 = st.columns([3, 2, 4, 1])
            _h1.markdown("<div style='font-size:10px;color:rgba(150,210,255,0.5);'>CİHAZ / SANTRAL</div>", unsafe_allow_html=True)
            _h2.markdown("<div style='font-size:10px;color:rgba(150,210,255,0.5);'>BAKIM TÜRÜ</div>", unsafe_allow_html=True)
            _h3.markdown("<div style='font-size:10px;color:rgba(150,210,255,0.5);'>SEBEP / DETAY</div>", unsafe_allow_html=True)

            for i, cihaz in enumerate(edit_list):
                _ad   = cihaz.get("ad", "")
                _tur  = cihaz.get("bakim_turu", "")
                _det  = cihaz.get("detay", "")
                c1, c2, c3, c4 = st.columns([3, 2, 4, 1])
                with c1:
                    st.text_input("ad",   value=_ad,  key=f"bk_ad_{i}",  label_visibility="collapsed", placeholder="Cihaz adı")
                with c2:
                    st.text_input("tur",  value=_tur, key=f"bk_tur_{i}", label_visibility="collapsed", placeholder="Bakım türü")
                with c3:
                    st.text_input("det",  value=_det, key=f"bk_det_{i}", label_visibility="collapsed", placeholder="Sebep / Detay")
                with c4:
                    if st.button("🗑️", key=f"bk_sil_{i}", help="Sil"):
                        st.session_state["bakim_edit_list"].pop(i)
                        # ilgili input key'lerini de temizle
                        for _k in [f"bk_ad_{i}", f"bk_tur_{i}", f"bk_det_{i}"]:
                            st.session_state.pop(_k, None)
                        st.rerun()
        else:
            st.caption("Henüz kayıtlı cihaz yok.")

        # ── Yeni cihaz ekle ──────────────────────────────
        st.markdown("---")
        st.markdown("<div style='font-size:11px;color:rgba(150,210,255,0.6);margin-bottom:4px;'>➕ Yeni cihaz ekle</div>", unsafe_allow_html=True)
        n1, n2, n3, n4 = st.columns([3, 2, 4, 1])
        with n1: yeni_ad  = st.text_input("Cihaz adı",  key="bk_yeni_ad",  label_visibility="collapsed", placeholder="Cihaz / Santral adı")
        with n2: yeni_tur = st.text_input("Bakım türü", key="bk_yeni_tur", label_visibility="collapsed", placeholder="Filtre, Arıza...")
        with n3: yeni_det = st.text_input("Sebep",      key="bk_yeni_det", label_visibility="collapsed", placeholder="Sebep / Detay")
        with n4:
            if st.button("➕", key="bk_ekle", help="Ekle") and yeni_ad.strip():
                st.session_state["bakim_edit_list"].append({
                    "ad":        yeni_ad.strip(),
                    "bakim_turu": yeni_tur.strip(),
                    "detay":     yeni_det.strip(),
                })
                for _k in ["bk_yeni_ad", "bk_yeni_tur", "bk_yeni_det"]:
                    st.session_state.pop(_k, None)
                st.rerun()
        ...
        with sv2:
            if st.button("💾 Kaydet", key="bk_kaydet", use_container_width=True):
                # Inline düzenlemeleri session_state'ten oku
                updated = []
                for i in range(len(edit_list)):
                    _a = st.session_state.get(f"bk_ad_{i}",  "").strip()
                    _t = st.session_state.get(f"bk_tur_{i}", "").strip()
                    _d = st.session_state.get(f"bk_det_{i}", "").strip()
                    if _a:
                        updated.append({"ad": _a, "bakim_turu": _t, "detay": _d})
                new_bakim = dict(bakim_ozet)
                new_bakim["bakim_cihazlari"] = updated
                new_bakim["toplam_bakim"]    = len(updated)
                try:
                    from supabase import create_client as _sc
                    _c = _sc(url, key)
                    _c.table("lokasyonlar").update(
                        {"bakim_ozet": new_bakim}
                    ).eq("lokasyon_id", lok_id).execute()
                    st.session_state["bakim_edit_list"] = updated
                    st.session_state["bakim_edit_lok"]  = lok_id
                    fetch_bakim_data.clear()
                    st.success(f"✅ {len(updated)} cihaz kaydedildi.")
                    st.rerun()
